...
public sealed class OverlordMenu(AgentProviderRegistry providers, WindowsTerminalLauncher wt)
{
            ...
            .Start("[cyan1]Refining your order...[/]", ctx =>
            {
                try
                {
                    var psi = new ProcessStartInfo("claude")
                    {
                        ...
                        // stderr is intentionally not redirected — we never drain it,
                        // and redirecting without draining risks deadlock when claude
                        // fills the OS pipe buffer while ReadToEnd() is blocking on stdout.
                    ...
                    // ReadToEnd() blocks until stdout closes. Give the process up to
                    // 60 s to fully exit; kill it on timeout so we don't leak a
                    // claude subprocess, and discard output from an abnormal run.
    ...
    /// <summary>
    /// The directory under which every MindAttic project repo lives. Walks up
    /// from the exe looking for <c>scripts/publish.ps1</c> (reliable when running
    /// from a checkout), then falls back to the historical path.
    /// </summary>
    public static string ResolveMindAtticRoot()
    {
        ...
        while (!string.IsNullOrEmpty(dir))
        {
            if (File.Exists(Path.Combine(dir, "scripts", "publish.ps1")))
                return Path.GetDirectoryName(dir) ?? dir; // parent of the launcher repo
